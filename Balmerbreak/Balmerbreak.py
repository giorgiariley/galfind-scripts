...
output_folder = "balmer_break_outputs"
...
for idx, galaxy in enumerate(cat):
    try:
        ...
        plt.plot(sed_wavs[valid], sed_mags, label='Best-fit SED (EAZY)', lw=2)
        ...
        plt.title(f"Best-fit SED for galaxy {idx} using {SED_fitter_arr[-1].label} at redshift {z:.2f}")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
        plot_path = os.path.join(output_folder/'SED_plots', f"galaxy_{idx:04d}_sed_plot.png")
